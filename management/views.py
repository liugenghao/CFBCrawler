...
def callForBidInfo(request, code):
    ...
    if code == '':
        ...
        if sec_menu_code:#如果含有二级菜单
            sec_active_code = sec_menu_code
            sec_menu = get_menu(sec_menu_code,3)
            cfb_informations = get_article(sec_menu_code)
        if thd_menu_code:#如果含有三级菜单
            cfb_informations = get_article(thd_menu_code)
            thd_active_code = thd_menu_code
    ...
    return render(request, 'call_for_bid_info.html',
                  {'cfb_informations': cfb_informations, 'top_cfb_menu': top_cfb_menu,'sec_menu':sec_menu,
                   ...
                   'itemNum': itemNum, 'total_num': total_num, 'action_url': action_url,'code': code,'active_code':active_code,'sec_active_code':sec_active_code,'thd_active_code':thd_active_code })
# 抓取
